...
tcSpliceExpr splice@(HsTypedSplice name expr) res_ty
  = addErrCtxt (spliceCtxtDoc splice) $
    ...
    ; res <- setStage (RunSplice modfinalizers_ref) $
        case stage of
          Splice {}            -> tcTopSplice expr res_ty
          Brack pop_stage pend -> tcNestedSplice pop_stage pend name expr res_ty
          RunSplice _          -> tcTopSplice expr res_ty
...
Since splices may occur nested, as in
...
we are careful to use a different reference in the local environment to collect
the finalizers of each splice.
